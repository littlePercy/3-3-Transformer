...
# data = load('imagenet21k+imagenet2012_R50+ViT-B_16.npz')
# lst = data.files
# for item in lst:
#     print(item)
#     #print(data[item]
...
'''
参考: https://zhuanlan.zhihu.com/p/418184940
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)  # [B, 196, 768] 输入Patch Embedding层                                                   √: patch嵌入 [B,196,768]
        # [1, 1, 768] -> [B, 1, 768] 在batch维度复制batch_size份
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  #                                                        √: 分类向量 [B,1,768]
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)  # [B, 197, 768] 将cls_token与x在维度1上拼接。注意：cls_token在前，x在后     √: cat [B,196+1,768]
        else:
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)

        x = self.pos_drop(x + self.pos_embed)  # concat后的数据加上position，再经过dropout层                                √：加上位置编码 [B,197,768]
        x = self.blocks(x)  # 经过堆叠的Encoder blocks                                                                    √：十二次Encoder后 [B,197,768]
        x = self.norm(x)  # Layer Norm层
        if self.dist_token is None:  # 一般为None，本质上是Identity层
            return self.pre_logits(x[:, 0])  # 提取cls_token信息，因为cls_token维度在前，所以索引为0就是cls本身                 √：[B,768]
        else:
            return x[:, 0], x[:, 1]

    def forward(self, x):
        x = self.forward_features(x) # [Batch_size,  embed_dim]                                                         √：[B,768]
...
if __name__ == "__main__":
    ...
    x = torch.FloatTensor(16,3,224,224).to(device)
    VisionTransformer = VisionTransformer().to(device)
    print(VisionTransformer)
    output = VisionTransformer(x) # 输出尺寸 [Batch_size, num_classes]
    print(output.shape)
    # Embedding = PatchEmbed().cuda()
    # Attention = Attention(dim=768).cuda()
    # y = Embedding(x) # [4, 196, 768]
    # att = Attention(y) # [4, 196, 768]
